    ...
    for (let pageNum = 1; pageNum <= pages.length; pageNum++) {
      ...
      for (const ann of pageAnnotations) {
        ...
        if (ann.type === 'sticker' && ann.sticker_url) {
          ...
          try {
            // Fetch the PNG image
            const imageResponse = await fetch(ann.sticker_url);
            if (!imageResponse.ok) {
              console.warn(`Failed to fetch annotation image: ${ann.sticker_url}`);
              continue;
            }
            ...
            try {
              ...
              try {
                ...
                console.warn(`Failed to embed image for annotation: ${ann.sticker_url}`, jpgError);
                continue;
              }
            }
            ...
            console.log(`Embedded annotation image on page ${pageNum}:`, ann.sticker_url);
          } catch (error) {
